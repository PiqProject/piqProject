...
public interface UserRepository extends JpaRepository<UserEntity, Long> {
        ...
        @Query(value = "SELECT u FROM UserEntity u WHERE " +
                        ...
                        "   u.id NOT IN :excludedUserIds AND " + // 2. 제외 목록에 있는 사용자 제외(관리자 포함)
                        ...
                        "       ) * 1.0" +
                        "       +" +
                        "       (" + // 6. [핵심 로직 변경] 나의 '이상형 조건'과 상대방의 '실제 특성' 일치 점수 계산 (가중치 1.5)
                        "           SELECT COUNT(matching_trait) FROM UserTraitEntity matching_trait " + // 상대방(u)의 실제
                                                                                                         // 특성(Trait)
                                                                                                         // 테이블을 조회
                        "           WHERE matching_trait.user = u AND matching_trait.traitOption IN (" + // 그 특성 옵션이 아래
                                                                                                         // 목록에 포함되는가?
                        ...
                        "       ) * 1.5" +
                        "   ) DESC, " + // -- 점수 계산 종료 --
                        "   u.id ASC" // 7. 점수가 같을 경우 ID 순으로 정렬 (일관된 순서 보장)
        )
